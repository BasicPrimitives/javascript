...
*/
primitives.common.jQueryButtonsTemplate=function(){var a,b=jQuery("<ul></ul>");b.css({position:"absolute"}).addClass("ui-widget ui-helper-clearfix");a=b.wrap("<div>").parent().html();var c=primitives.common.hashCode(a);return{template:function(){return a},getHashCode:function(){return c},render:function(a,c){var b=jQuery(c.element),h=0,e,g=c.buttons,f,j,i;switch(c.renderingMode){case primitives.common.RenderingMode.Create:for(i=0;i<g.length;i+=1)e=g[i],j=jQuery('<li data-buttonname="'+e.name+'"></li>').css({position:"absolute",
top:h+"px",left:"0px",width:e.size.width+"px",height:e.size.height+"px",padding:"3px"}).addClass("orgdiagrambutton"),b.append(j),f={icons:{primary:e.icon},label:e.label},""!=e.text&&(f.text=e.text),primitives.common.isNullOrEmpty(e.tooltip)||j.attr("title",e.tooltip),j.button(f),h+=10+e.size.height}}}};primitives.orgdiagram.Widget=function(a,b,c,d){this.widgetEventPrefix="orgdiagram";this.options=new a;this.control=null;this.taskManagerFactory=b;this.eventArgsFactory=c;this.templates=d};
primitives.orgdiagram.Widget.prototype._create=function(){this.element.addClass("ui-widget");this.control=primitives.orgdiagram.BaseControl(this.element[0],this._readOptions(this.options),this.taskManagerFactory,this.eventArgsFactory,this.templates)};primitives.orgdiagram.Widget.prototype.destroy=function(){this.control.destroy()};primitives.orgdiagram.Widget.prototype.update=function(a,b){this.control.update(a,b)};
