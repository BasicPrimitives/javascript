/*
 Basic Primitives Diagrams v5.6.0
...
return b};a.famdiagram.FamilyBalance.prototype.balanceOrgTree=function(g,b,d,c,f){var e,h,i,j,k,m,n,l,p,q,r;g.loopLevels(this,function(s,t){var w=a.common.graph(),v={},A=null,x;n=[];g.loopChildren(this,t.id,function(a,c,d){var f;null==A&&(A=c);v[c.id]={};if(b.hasOwnProperty(c.id))for(f in m=b[c.id],m)if(m.hasOwnProperty(f)){q=m[f];v[c.id][f]={};e=0;for(h=q.length;e<h;e+=1)k=q[e],x=k.commonParent==t.id?k.toParent:g.node(k.fromParent).childIndex<g.node(k.toParent).childIndex?"__right__":"__left__",
c.id!=x&&(w.addEdge(c.id,x,{weight:0}),w.edge(c.id,x).weight+=1,null==v[c.id][f][x]&&(v[c.id][f][x]=0),v[c.id][f][x]+=1)}0<d&&w.addEdge(c.id,A.id,{weight:0})});if(null!=A){r=[];w.getTotalWeightGrowthSequence(this,function(a){return a.weight},function(a){r.push(a)});0===r.length&&(r=[A.id]);l=this.balanceItems(r,"__left__","__right__",v,d,c,f);i=0;for(j=l.length;i<j;i+=1)p=g.node(l[i]),p.childIndex=i,n.push(p.id)}g.arrangeChildren(t.id,n)})};a.famdiagram.FamilyBalance.prototype.balanceItems=function(g,
b,d,c,f,e,h){var i=[],j,k=a.common.LinkedHashItems(),m=0,n={},l,p,q,r={},s,t,w,v,A,x,B,z,y,u,F,E,D,C,G,I,H,K;p=[b,null,d];for(j=0;j<p.length;j+=1)s=p[j],l=m++,q=new a.famdiagram.Slot(s),q.position=j,k.add(l,q),null!==s&&(r[s]=l);for(j=0;j<g.length;j+=1)if(s=g[j],s!=b&&s!=d)for(C in p=l=B=x=A=v=w=t=null,h.hasOwnProperty(s)&&(q=h[s],n.hasOwnProperty(q)&&(p=e[s],q=n[q],l=q.previousContext(p),p=q.nextContext(p))),k.iterate(function(a,b){var d,g,e,h;if(null==a.itemId){E=f[s];F=u=y=z=0;for(d in a.crossings)a.crossings.hasOwnProperty(d)&&
(E&&null!=E[d]&&(z+=a.crossings[d]*E[d]),F+=a.crossings[d]);for(d in c[s])if(c[s].hasOwnProperty(d))for(g in e=c[s][d],e)e.hasOwnProperty(g)&&r.hasOwnProperty(g)&&(h=k.item(r[g]),null!=h&&(h.position<a.position?(z+=(a.left[d]||0)-(h.left[d]||0),u+=Math.abs(h.balance+1)):(z+=(a.right[d]||0)-(h.right[d]||0),u+=Math.abs(h.balance-1)),y+=Math.abs(h.position-a.position)));if(null==v||v>z||v==z&&(A>y||A==y&&(x>u||x==u&&B>F)))t=b,v=z,w=a,A=y,x=u,B=F}},l,p),k.insertBefore(t,m++,w.clone(null)),D=r[s]=m,l=
w.clone(s),e.hasOwnProperty(s)&&(q=h[s],n.hasOwnProperty(q)||(n[q]=a.common.SortedList()),q=n[q],q.add(e[s],m)),k.insertBefore(t,m++,l),H=l.position=0,k.iterate(function(a,b){var c,d;if(b!=D){d=f[s];for(c in d)d.hasOwnProperty(c)&&(a.left[c]=a.left[c]?a.left[c]+d[c]:d[c]);H+=1;a.position=H}},D),H=0,k.iterateBack(function(a,b){var c,d;if(b!=D){d=f[s];for(c in f[s])f[s].hasOwnProperty(c)&&(a.right[c]=a.right[c]?a.right[c]+d[c]:d[c]);H-=1;a.position=H}},D),c[s])if(c[s].hasOwnProperty(C))for(I in G=c[s][C],
G)G.hasOwnProperty(I)&&r.hasOwnProperty(I)&&(K=r[I],q=k.item(K),null!=q&&(0>q.position?(q.balance+=1,l.balance-=1,k.iterateBack(function(a,b){b!=D&&b!=K&&(a.crossings[C]=a.crossings[C]?a.crossings[C]+G[I]:G[I])},D,K)):(q.balance-=1,l.balance+=1,k.iterate(function(a,b){b!=D&&b!=K&&(a.crossings[C]=a.crossings[C]?a.crossings[C]+G[I]:G[I])},D,K))));k.iterate(function(a){a=a.itemId;null!=a&&(a!=b&&a!=d)&&i.push(a)});return i};a.famdiagram.FamilyBalance.prototype.ExtraGravity=function(a){this.toParent=
...
new a.famdiagram.EdgeItem(c,c,m.fromItem,m.fromItem));return b.SKIP}));for(n in g)if(g.hasOwnProperty(n)){f=g[n];f.sort(function(a,b){return b.toItems.length-a.toItems.length});for(l=0;l<f.length;l+=1){m=f[l];for(var p=b,q=d,r=m.fromItem,s=m.toItems,t=void 0,w=!0,v=null,A=void 0,x=void 0,B=void 0,z=void 0,y=[],x=0,B=s.length;x<B;x+=1)if(A=s[x],t=q.edge(r,A),null!=t){if(null==v)v=t.getFar(A);else if(v!=t.getFar(A)){w=!1;break}y.push(t.getNear(A))}else{w=!1;break}if(w&&(z=new a.famdiagram.FamilyItem({id:m.id,
isVisible:!0,isLevelNeutral:!0,isActive:!1,itemConfig:m}),p.bundleParents(v,y,z.id,z)?(z.levelGravity=2,w=!0):p.bundleChildren(v,y,z.id,z)?(z.levelGravity=1,w=!0):p.bundleParents(v,s,z.id,z)?(z.levelGravity=2,w=!0):p.bundleParents(v,s,z.id,z)&&(z.levelGravity=1,w=!0),w)){x=0;for(B=s.length;x<B;x+=1)A=s[x],t=q.edge(r,A),t.setFar(A,z.id)}}}}}var c=null,f=null;return{process:function(a){var h=b.getLogicalFamily(),i=g.getAnnotations(),h=h.clone();d(h,i);c=h;f=g.getMaximumId();if(a&&!h.validate())throw"References are broken in family structure!";
